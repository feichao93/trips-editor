import { h } from '@cycle/dom'
import { add, identity } from 'ramda'
import { ItemRecord } from './Item'
import { Point, ResizeDirConfig } from '../interfaces'
import { containsPoint, getCoordinateUpdater } from './common'
import { List } from 'immutable'

const ImgItem = ItemRecord(
  'ImgItem',
  {
    id: -1,
    locked: false,
    x: 0,
    y: 0,
    width: 0,
    height: 0,
    naturalWidth: 0,
    naturalHeight: 0,
    opacity: 1,
    url: '',
  },
  {},
)

export const imgItem = ImgItem()
type ImgItem = typeof imgItem

ImgItem.prototype.resize = function resize(
  this: ImgItem,
  anchor: Point,
  { h, v }: ResizeDirConfig,
  startPos: Point,
  endPos: Point,
) {
  if (this.locked) {
    return this
  }
  const vertices = this.getVertices()
  const ratio = this.height / this.width
  const xUpdater = h ? getCoordinateUpdater(anchor.x, startPos.x, endPos.x) : identity
  const yUpdater = v ? getCoordinateUpdater(anchor.y, startPos.y, endPos.y) : identity

  const updatedVertices = vertices.map(p => ({ x: xUpdater(p.x), y: yUpdater(p.y) }))

  const nextX = updatedVertices.map(p => p.x).min()
  const nextY = updatedVertices.map(p => p.y).min()
  const nextWidth = updatedVertices.map(p => p.x).max() - nextX
  const nextHeight = updatedVertices.map(p => p.y).max() - nextY
  // TODO Use a correct implementation
  return this.set('x', nextX)
    .set('y', nextY)
    .set('width', Math.max(nextWidth, nextHeight / ratio))
    .set('height', Math.max(nextHeight, nextWidth * ratio))
}

ImgItem.prototype.move = function move(this: ImgItem, dx: number, dy: number) {
  if (this.locked) {
    return this
  }
  return this.update('x', add(dx)).update('y', add(dy))
}

ImgItem.prototype.render = function render(this: ImgItem) {
  return h('image', {
    key: this.id,
    attrs: {
      x: this.x,
      y: this.y,
      href: this.url,
      width: this.width,
      height: this.height,
      opacity: this.opacity,
    },
  })
}

ImgItem.prototype.getVertices = function getVertices(this: ImgItem) {
  return List([
    { x: this.x, y: this.y },
    { x: this.x + this.width, y: this.y },
    { x: this.x + this.width, y: this.y + this.height },
    { x: this.x, y: this.y + this.height },
  ])
}

ImgItem.prototype.containsPoint = function _containsPoint(this: ImgItem, p: Point) {
  return containsPoint(this.getVertices().toArray(), p)
}

export default ImgItem
